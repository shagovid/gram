...
public class GetFollowingsPostsQueryHandler
    : IRequestHandler<GetFollowingsPostsQuery, GetFollowingsPostsResponseDTO>
{
    private readonly ICurrentAuth _auth;
    private readonly ObjectsStorageService _objectsStorage;
    private readonly IAppDbContext _context;

    public GetFollowingsPostsQueryHandler(
        ICurrentAuth auth,
    ...
    public async Task<GetFollowingsPostsResponseDTO> Handle(
        GetFollowingsPostsQuery request,
        CancellationToken cancellationToken)
    {
        ...
        query = query
            .Where(x => x.Owner.Followers.Any(y => y.FollowerId == _auth.Id));
        ...
        return new GetFollowingsPostsResponseDTO(
...
public class GetFollowingsPostsResponseDTO
{
    public List<PostBaseDTO> Posts { get; }

    public GetFollowingsPostsResponseDTO(List<PostBaseDTO> posts)
    {
        Posts = posts;
    }
}